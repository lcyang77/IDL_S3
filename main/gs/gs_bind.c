...
// 定义事件组和事件标志
static EventGroupHandle_t bind_event_group;
const int START_AP_BIT = BIT0;
    ...
    if(base_event == GS_WIFI_EVENT){
        ...
        case GS_WIFI_EVENT_SCAN_DONE:
            if(g_bind_timeout_timer_handle != NULL){
                // 设置事件标志位，通知绑定任务处理
                xEventGroupSetBits(bind_event_group, START_AP_BIT);
            }
            break;
        default:
            break;
...
static void gs_bind_task(void *arg){
    EventBits_t uxBits;
    for(;;){
        // 等待事件标志被设置
        uxBits = xEventGroupWaitBits(bind_event_group, START_AP_BIT, pdTRUE, pdFALSE, portMAX_DELAY);

        if(uxBits & START_AP_BIT){
            // 在这里执行原先的 __ap_bind_cfg_ap_server_start() 函数
...
void gs_bind_init(void){
    ...
    // 创建事件组
    bind_event_group = xEventGroupCreate();

    // 创建绑定任务
    xTaskCreate(gs_bind_task, "gs_bind_task", 4096, NULL, 5, NULL);
}
