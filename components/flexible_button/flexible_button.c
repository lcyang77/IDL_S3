...
int32_t flex_button_register(flex_button_t *button)
{
    ...
    */
    ...
    button->debounce_last_scan_cnt = 0;
...
/**
 * @brief Delete all button
 * 
 * @param void
 * @return none
*/
void flex_button_delete_all(void){
    btn_head = NULL;
    button_cnt = 0;
    g_btn_status_reg = (btn_type_t)0;
}
...
*/
static uint8_t flex_button_process(uint8_t interval)
{
    uint8_t i;
    ...
    for (target = btn_head, i = button_cnt - 1; target != NULL; target = target->next, i--)
    {
        ...
        case FLEX_BTN_STAGE_DEFAULT: /* stage: default(button up) */
            if (BTN_IS_PRESSED(i)) /* is pressed */
            {
                target->scan_cnt = 0;
                target->click_cnt = 0;
                target->debounce_last_scan_cnt = 0;
        ...
        case FLEX_BTN_STAGE_DOWN: /* stage: button down */
            if (BTN_IS_PRESSED(i)) /* is pressed */
            {
                target->debounce_last_scan_cnt = target->scan_cnt;
            ...
            {
                if(target->scan_cnt - target->debounce_last_scan_cnt <= target->debounce_tick){
                    break;
                }else{
                    target->debounce_last_scan_cnt = target->scan_cnt;
                }
 ...
 * @return Activated button count
*/
uint8_t flex_button_scan(uint8_t interval)
{
    flex_button_read();
    return flex_button_process(interval);
}
