...
int http_ssl_conn_wrapper(http_client_t *client, const char *host)
{
    ...
    if (!client->ssl) {
        http_err("Memory http_malloc error.");
        ret = -1;
        goto exit;
    }
...
int http_ssl_close_wrapper(http_client_t *client)
{
    ...
    mbedtls_net_http_free(&ssl->net_ctx);
#ifdef MBEDTLS_X509_CRT_PARSE_C
    mbedtls_x509_crt_http_free(&ssl->cacert);
    mbedtls_x509_crt_http_free(&ssl->clicert);
#endif
    mbedtls_pk_http_free(&ssl->pkey);
    mbedtls_ssl_http_free(&ssl->ssl_ctx);
    mbedtls_ssl_config_http_free(&ssl->ssl_conf);
#ifdef MBEDTLS_ENTROPY_C
    mbedtls_ctr_drbg_http_free(&ssl->ctr_drbg);
    mbedtls_entropy_http_free(&ssl->entropy);
#endif
    http_free(ssl);
    ssl = NULL;
    return 0;
