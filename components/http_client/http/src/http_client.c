...
static int http_client_parse_url(const char *url, char *scheme, size_t max_scheme_len, char *host, size_t maxhost_len, int *port, char *path, size_t max_path_len)
{
...
static int http_client_get_info(http_client_t *client, char *send_buf, int *send_idx, char *buf, size_t len)   /* 0 on success, err code on failure */
{
    int ret ;
    int cp_len ;
    int idx = *send_idx;
...
static int http_client_send_header(http_client_t *client, const char *url, int method, http_client_data_t *client_data)
{
    ...
    char *host = NULL;
    char *path = NULL;
    int host_size = HTTP_CLIENT_MAX_HOST_LEN;
    int path_size = HTTP_CLIENT_MAX_URL_LEN;
    int len, formdata_len;
    int total_len = 0;
    char *send_buf = NULL;
    char *buf = NULL;
    int send_buf_size = HTTP_CLIENT_SEND_BUF_SIZE;
    int buf_size = HTTP_CLIENT_SEND_BUF_SIZE;
    ...
    if (!host) {
        http_err("host http_malloc failed");
        ret = HTTP_ENOBUFS;
        goto exit;
    }
    ...
    if (!path) {
        http_err("path http_malloc failed");
        ret = HTTP_ENOBUFS;
        goto exit;
    }
    ...
    if (!send_buf) {
        http_err("send_buf http_malloc failed");
        ret = HTTP_ENOBUFS;
        goto exit;
    }
    ...
    if (!buf) {
        http_err("buf http_malloc failed");
        ret = HTTP_ENOBUFS;
        goto exit;
    }
    ...
    int res = http_client_parse_url(url, scheme, sizeof(scheme), host, host_size, &(port), path, path_size);
    if (res != HTTP_SUCCESS) {
        http_err("http_client_parse_url returned %d", res);
        ret = res;
        goto exit;
    }
    ...
    if ((formdata_len = http_client_formdata_len(client_data)) > 0) {
...
{
    int ret = 0;
    int timeout_ms = HTTP_CLIENT_MAX_RECV_WAIT_MS;
    ...
    if (client_data->response_content_len == -1 && client_data->is_chunked == false) {
        while(true)
        {
            ...
            if (count + len < client_data->response_buf_len - 1) {
                ...
                count += len;
                client_data->response_len = count;
                client_data->response_buf[count] = '\0';
            } else {
                memcpy(client_data->response_buf + count, data, client_data->response_buf_len - 1 - count);
                client_data->response_len = client_data->response_buf_len - 1;
        ...
        if (crlf_ptr == NULL) {
            if ( len < HTTP_CLIENT_CHUNK_SIZE - 1 ) {
                ...
                if (HTTP_CLIENT_CHUNK_SIZE - len - 1 <= 0) {
                    http_err("%s %d error max_len %d", __func__, __LINE__, HTTP_CLIENT_CHUNK_SIZE - len - 1);
                    return HTTP_EUNKOWN;
                }
        ...
        if (colon_ptr) {
            ...
            if (0 == strncasecmp(key_ptr, "Content-Length", key_len)) {
                ...
                if ( HTTP_CLIENT_MAX_URL_LEN < value_len + 1 ) {
                    http_err("url is too large (%d >= %d)", value_len + 1, HTTP_CLIENT_MAX_URL_LEN);
                    return HTTP_EUNKOWN;
                }

                if(client_data->redirect_url == NULL) {
                    client_data->redirect_url = (char* )http_malloc(HTTP_CLIENT_MAX_URL_LEN);
                }
...
HTTPC_RESULT http_client_conn(http_client_t *client, const char *url)
{
    ...
    /* First we need to parse the url (http[s]://host[:port][/[path]]) */
    int res = http_client_parse_url(url, scheme, sizeof(scheme), host, host_size, &(client->remote_port), path, path_size);
    if (res != HTTP_SUCCESS) {
        http_err("http_client_parse_url returned %d", res);
        ret = res;
        goto exit;
...
exit:
    ...
    http_debug("http_client_conn() result:%d, client:%p", ret, client);
...
HTTPC_RESULT http_client_send(http_client_t *client, const char *url, int method, http_client_data_t *client_data)
{
    ...
    http_debug("http_client_send() result:%d, client:%p", ret, client);
    return (HTTPC_RESULT)ret;
}
...
HTTPC_RESULT http_client_recv(http_client_t *client, http_client_data_t *client_data)
{
    int reclen = 0;
    int ret = HTTP_ECONN;
    // TODO: header format:  name + value must not bigger than HTTP_CLIENT_CHUNK_SIZE.
    char *buf = NULL;

    if (client_data->header_buf_len < HTTP_CLIENT_CHUNK_SIZE ||
        client_data->response_buf_len < HTTP_CLIENT_CHUNK_SIZE) {
        http_err("Error: header buffer or response buffer should not less than %d!", HTTP_CLIENT_CHUNK_SIZE);
        ret = HTTP_EARG;
        goto exit;
    }

    buf = (char *) http_malloc(HTTP_CLIENT_CHUNK_SIZE);
    if (!buf) {
        http_err("http_malloc failed socket fd %d!", client->socket);
        ret = HTTP_ENOBUFS;
        goto exit;
    }
    ...
    if (client_data->is_more) {
        ...
        ret = http_client_recv_data(client, buf, 1, HTTP_CLIENT_CHUNK_SIZE - 1, &reclen);
    ...
    http_debug("http_client_recv_data() result:%d, client:%p", ret, client);
...
void http_client_clse(http_client_t *client)
{
    ...
    client->socket = -1;
    http_debug("http_client_clse() client:%p", client);
}
...
HTTPC_RESULT http_client_prepare(http_client_data_t *client_data, int header_size, int resp_size)
{
    ...
    if (header_size < HTTP_CLIENT_CHUNK_SIZE || resp_size < HTTP_CLIENT_CHUNK_SIZE) {
        http_err("Error: header buffer or response buffer should not less than %d!", HTTP_CLIENT_CHUNK_SIZE);
        return HTTP_EARG;
    }

    memset(client_data, 0, sizeof(http_client_data_t));
