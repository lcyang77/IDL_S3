...
[更正文档](https://gitee.com/alios-things/http/edit/master/README.md) &emsp;&emsp;&emsp;&emsp; [贡献说明](https://help.aliyun.com/document_detail/302301.html)
...
> HTTP_CLIENT_MAX_RECV_WAIT_MS ： 接收超时时间
```C
...
# API说明
- 参考 [aos_http_client_api](https://g.alicdn.com/alios-things-3.3/doc/group__aos__http_client__api.html)
...
## 向URL发送GET请求，等待回复。
```C
HTTPC_RESULT http_client_get(http_client_t *client, char *url, http_client_data_t *client_data)
```
...
## 向URL发送POST请求，等待回复。
```C
HTTPC_RESULT http_client_post(http_client_t *client, char *url, http_client_data_t *client_data)
```
...
## 向URL发送PUT请求，等待回复。
```C
HTTPC_RESULT http_client_put(http_client_t *client, char *url, http_client_data_t *client_data)
```
...
## 向URL发送DELETE请求，等待回复。
```C
HTTPC_RESULT http_client_delete(http_client_t *client, char *url, http_client_data_t *client_data)
```
