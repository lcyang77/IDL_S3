...
/** @brief   This structure defines the http_client ssl structure.  */
typedef struct {
 ...
 *  @param[in] client    pointer to the http_client.
...
int http_tcp_conn_wrapper(http_client_t *client, const char *host)
{
    ...
    if ((client->socket = socket(AF_INET, SOCK_STREAM, 0)) < 0) {
        http_err("socket < 0, return HTTP_ECONN");
        return HTTP_ECONN;
    }
    ...
    if (!hostinfo) {
        http_err("hostinfo is null, return EDNS");
        return HTTP_EDNS;
    }
    ...
    dest.sin_port = htons(client->remote_port);
    memcpy((void *)&dest.sin_addr.s_addr, (void *)hostinfo->h_addr, (size_t)hostinfo->h_length);

    char ip[4];
    memcpy(ip, &dest.sin_addr.s_addr, 4);

    http_client_cb_t *client_cb = client->client_cb;
    if(client_cb != NULL){
        if(client_cb->event_cb != NULL){
            client_cb->event_cb(client, HTTP_EVENT_DNS_GET_IP, ip);
        }
    }

    if (connect(client->socket, (struct sockaddr *)&dest, sizeof(dest)) != 0) {
        close(client->socket);
        http_err("connect fail, return HTTP_ECONN");
        return HTTP_ECONN;
    }else{
        http_info("connected");
    }
...
{
    int written_len = 0;
    if((int)data > 0x60700000){
    // if(0){
        char g_send_buf[1472] = "";
        while (written_len < length) {
            int len = ((length - written_len)>1472)?1472:(length - written_len);
            memcpy((void *)g_send_buf, (void *)data + written_len, len);
            int ret = send(client->socket, g_send_buf, len, 0);
            if (ret > 0) {
                written_len += ret;
                continue;
            } else if (ret == 0) {
...
int http_ssl_send_wrapper(http_client_t *client, const char *data, size_t length)
{
    ...
    http_client_ssl_t *ssl = (http_client_ssl_t *) client->ssl;
    if (!ssl) {
    	return -1;
    }
...
static void http_client_debug( void *ctx, int level, const char *file, int line, const char *str )
{
    http_debug("%s", str);
}

static int http_client_random(void *prng, unsigned char *output, size_t output_len)
{
...
int http_ssl_conn_wrapper(http_client_t *client, const char *host)
{
    ...
    client->ssl = (http_client_ssl_t *)calloc(1, sizeof(http_client_ssl_t));
    ...
    ssl = (http_client_ssl_t *)client->ssl;
    ...
    */
    ...
    mbedtls_ssl_conf_rng(&ssl->ssl_conf, http_client_random, &ssl->ctr_drbg);
    mbedtls_ssl_conf_dbg(&ssl->ssl_conf, http_client_debug, NULL);
